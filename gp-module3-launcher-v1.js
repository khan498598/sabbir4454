(function () {
  ...
  if (window.__GP_MODULE3_LAUNCHER_V1__) return;
  window.__GP_MODULE3_LAUNCHER_V1__ = true;

  var VERSION = 'r386-all-module-history-navigation-v1';
  var WINDOW_NAME = 'guardeer_module3_delta_profile';
  ...
  function moduleWindowUrl() {
    ...
    try {
      var target = new URL('module3-delta-profile.html', window.location.href);
      ...
      target.hash = 'module3-delta-profile';
      return target.toString();
    } catch (_) {
      return 'module3-delta-profile.html?build=' + BUILD + '#module3-delta-profile';
    }
  }
  ...
  function ensureLauncher() {
    ...
    var launcher = qs('[data-gp-module3-open]', grid);
    ...
    launcher.className = 'gp-step44-prime-tools-action gp-module3-prime-card';
    launcher.setAttribute('data-gp-module3-open', 'true');
    launcher.setAttribute('href', moduleWindowUrl());
    launcher.setAttribute('target', WINDOW_NAME);
    launcher.setAttribute('aria-label', 'Open MODULE-3 Delta and Profile Analytics in a new window');
    launcher.setAttribute('aria-haspopup', 'dialog');

    if (!qs('[data-gp-module3-launcher-title]', launcher)) {
      launcher.innerHTML = '<span>M3</span><div><strong data-gp-module3-launcher-title>MODULE-3 Delta &amp; Profile Analytics</strong><small>Three live delta and profile tools in one workspace.</small></div><em>Open</em>';
    }

    var moduleTwo = qs('[data-gp-module2-open]', grid);
    var moduleOne = qs('[data-gp-module1-open]', grid);
    var predecessor = moduleTwo || moduleOne;
    if (predecessor) {
  ...
  function openModuleWindow() {
    ...
    var width = Math.min(1740, Math.max(1040, availableWidth - 70));
    var height = Math.min(1060, Math.max(700, availableHeight - 70));
    ...
    observer = new MutationObserver(function (mutations) {
      var touchesPrimeTools = mutations.some(function (mutation) {
        var relevantNode = function (node) {
          ...
          return (node.matches && node.matches('#gp-step44-prime-tools-popover,.gp-step44-prime-tools-popover__grid,[data-gp-module1-open],[data-gp-module2-open],[data-gp-module3-open]'))
            || (node.querySelector && node.querySelector('#gp-step44-prime-tools-popover,.gp-step44-prime-tools-popover__grid,[data-gp-module1-open],[data-gp-module2-open],[data-gp-module3-open]'));
        };
  ...
  function onDocumentClick(event) {
    ...
    var launcher = target.closest('[data-gp-module3-open]');
    if (!launcher) return;
    launcher.setAttribute('href', moduleWindowUrl());
